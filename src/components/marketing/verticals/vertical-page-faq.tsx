...
import type { VerticalFaqItem } from "@/content/marketing/subscription-verticals";
import { cn } from "@/lib/utils";

export function VerticalPageFaq({
  title,
  items,
  className,
}: {
  title: string;
  items: readonly VerticalFaqItem[];
  className?: string;
}) {
  return (
            ...
            >
              ...
              <AccordionContent>
                <p className="text-muted-foreground pb-6 text-[15px] leading-relaxed tracking-[-0.01em] md:pb-7">
                  <MarketingRichText text={item.answer} />
                </p>
              </AccordionContent>
            </AccordionItem>
          ))}
